...
static vaddr_t* csr_address(word_t csr_addimm)
{
  switch (csr_addimm)
  {
    case 0x300:
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 00000 00001 000 00000 11001 11", ret    , I, s->dnpc = src1 - (src1 & 1));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = (*csr_address(imm)); (*csr_address(imm)) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = (*csr_address(imm)); (*csr_address(imm)) |= src1);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd) = (*csr_address(imm)); (*csr_address(imm)) &= (~(src1)));
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, s->dnpc = cpu.csr.mepc+4;cpu.csr.mstatus = (cpu.csr.mstatus&0x80)?(cpu.csr.mstatus|0x8):((cpu.csr.mstatus&(~0x8))|0x80);restore_interrupt(););
